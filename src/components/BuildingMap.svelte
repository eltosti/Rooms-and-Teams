<script>
  ...
  let roomst
  $: roomst = ($save[$save["activeBuilding"]]) ? $save[$save["activeBuilding"]].rooms : []
  let b = {...size}
  ...
  let selected = "#000000"
  let existingCells
  $: existingCells = $save["activeBuilding"] !== "" ? $save[$save["activeBuilding"]].existingCells : []
...
<div class="con">
    <div style="height: 6.5vh; display: flex; overflow: auto">
        <div class:selected={selected === "#000000"}
             style="flex-shrink: 0;margin-left: 0.5em;height: 1.6em;width: 1.6em; background-color: #000000"
             on:click={()=> {$selectedColor = "#000000";selected = "#000000"} }>
        </div>
        {#each roomst as room}
            <div class:selected={selected === room.color}
                 style="flex-shrink: 0;margin-left: 0.5em;height: 1.6em;width: 1.6em;background-color: {room.color}"
                 on:click={()=> {$selectedColor = room.color;selected = room.color}}>
            </div>
        {/each}
    </div>
    <div style="height: 85.5vh;width: 50vw; overflow: auto; display: flex; flex-direction: column; align-content: center">
        {#each existingCells as y,iy}
            <div class="row">
                {#each y as x,ix}
                    <div>
                        {#if $save[$save["activeBuilding"]].existingCells[iy][ix] === 1}
                            <MapCell {getColor} size={cellSize} cellId={ix+"-"+iy}
                                     cellInitialColor={getInitialColor(ix,iy)}/>
                        {:else}
                            <MapCell {getColor} size={cellSize} cellId={ix+"-"+iy}/>
                        {/if}
                    </div>
